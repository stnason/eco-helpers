...
{{-- Made the decision to have NO LINKS for any items that can have children (but might not):
     ...
     <a class="eco-hlpers dropdown-item">{{$menu_item->name}}</a>

     If we want a link then use the same format as the other non-children-bearing menu items:
     <a class="eco-helpers dropdown-item" href="{{config('app.url')}}/{{$menu_item->route}}">{{$menu_item->name}}</a>
       --}}
@if($menu_item->type == "module" || $menu_item->type == "submenu")
    ...
    @if(count($menu_item->children) > 0)
        <li class="eco-helpers dropdown-submenu">               {{-- css will provide the right arrow for the flyout menu. --}}
    @else
        {{-- <li class="eco-helpers dropdown-item">                   THIS INDENTS No right arrow for the flyout menu. --}}
        <li class="eco-helpers dropdown-submenu-no-flyout">     {{-- No right arrow for the flyout menu. --}}
            @endif

            <a class="eco-helpers dropdown-item">{{$menu_item->name}}</a>
            <ul class="eco-helpers dropdown-menu">
        ...
        @else
            {{-- <li><a class="eco-helpers dropdown-item" href="{{config('app.url')}}/{{$menu_item->route}}">{{$menu_item->name}}</a></li> --}}
            ...
            @if (Route::has($menu_item->route))
                <li><a class="eco-helpers dropdown-item" href="{{route($menu_item->route)}}">{{$menu_item->name}}</a></li>
            @else
                ...
                @if (Route::has($menu_item->route.'.index'))
                    ...
                    <li><a class="eco-helpers dropdown-item" href="{{route($menu_item->route.'.index')}}">{{$menu_item->name}}</a></li>
                @else
                    {{-- Okay, has no route "as is" or by resource.index. So, no link on this item. --}}
                    <li><a class="eco-helpers dropdown-item" href="#">{{$menu_item->name}}</a></li>
                @endif
