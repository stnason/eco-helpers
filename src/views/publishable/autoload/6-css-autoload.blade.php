{{--
 * The autoload file associated with the eco-helpers config file array
 * entry for 'auto_loaders' => [6 => 'name']
 *
 *  Usage (available in the controller as):
 *     ehLayout::setAutoload('name'); or ehLayout::setAutoload(6);
 *
 --}}

{{--
    https://cdn.datatables.net
    https://datatables.net/download/

    --}}
<link href="https://cdn.datatables.net/v/dt/jszip-2.5.0/dt-1.13.3/b-2.3.5/b-html5-2.3.5/b-print-2.3.5/fc-4.2.1/datatables.min.css" rel="stylesheet"/>

{{-- Add additional components if/when needed.
<link rel="stylesheet" type="text/css" href="{{ 'some-path' }}/extensions/Buttons/css/buttons.dataTables.css">
<link rel="stylesheet" type="text/css" href="{{ 'some-path' }}/extensions/FixedColumns/css/fixedColumns.dataTables.css">
--}}

