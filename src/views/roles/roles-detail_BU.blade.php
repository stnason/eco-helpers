                ...
                <div class="col-md-8">
                    ...
                    <div class="row">
                        <div class="col-md">
                            ...
                            <div class="form-group d-inline-flex">
                                {{--
                                {!! $control::label(['field_name'=>'act_as_role', 'display_name'=>$role, 'errors'=>$errors]) !!}
                                {!! $control::radio(['field_name'=>'act_as_role', 'model'=>$role,  'alert_if'=>'1', 'alert_class'=>'alert-info', 'errors'=>$errors, 'radio'=>[1=>'Yes', 0=>'No']]) !!}
                                --}}
                            </div>
                        </div>
                        <div class="col-md">
...
@section ('base_js')
    ...
    <script type="text/javascript">
        ...
        $("#remove-selected-button").click(function() {
            ...
            if ($(".tree-view input:checkbox:checked").length == 0) {
                ...
                if ((confirm(confirm_message))) {
                    ...
                    // The role id doesn't change so setting once here outside of the checkbox loop
                    var role_id = {{$role->id}};

                    {{-- Loop through all of the selected users and delete each one of them --}}

                    //$(".tree-view input:checkbox:checked").each(function () {
                    $(".tree-view input:checkbox").each(function () {
                       if (this.checked) {
                           var user_id = this.value;        // Pull the user id form the checkbox value.

                           //alert(user_id+" - "+{{$role->id}});
                    ...
                    // Refresh the page once after the loop runs through.
         ...
         * used in the checkbox loop delete users from the role lookup table
         * but it needs to wait for success on each one or the js will overrun the back-end and not delete all for larger sets.
         *
         * @returns {Promise<unknown>}
         */
        function removeUserFromRole(user_id,role_id) {
            return new Promise(function(resolve, reject, user_id, role_id) {
                $.ajaxSetup({
                    headers: {
                ...
                var url = "{{config('app.url')}}/delete-user-from-role/"+user_id+"/"+role_id;
                $.ajax({
                    type: "delete",
                    url: url,
                    ...
                    // TODO: if we're deleting a whole bunch this should probably me moved outside of this loop.
                    // maybe keep track of the number of deleted users here?
                    // location.reload();      // refresh the page to remove the deleted item and refresh the flash message.
                });
            });
        }

        removeUserFromRole().then(function(data) {
